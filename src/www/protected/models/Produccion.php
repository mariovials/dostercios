 ...
 */
class Produccion extends DTActiveRecord {

  const ID = 12;
  const ALIAS = 'pr';

  protected $default_controller_name = 'produccion';
  ...
  public function rules() {
    ...
    return array(
      array('titulo, url, categoria_id', 'required'),
      array('url', 'unique'),
  ...
  public function filesAttributes() {
    return array(
      'miniatura'=>"/img/produccion/{$this->id}/",
    );
  }
  ...
  {
    if ($this->isNewRecord) {
      mkdir(Yii::getPathOfAlias('webroot') . '/img/produccion/' . $this->id);
    }
  ...
  public function afterDelete()
  {
    ...
    rmdir(Yii::getPathOfAlias('webroot') . '/img/produccion/' . $this->id);
  ...
  {
    return CHtml::ListData(Categoria::model()->findAll(), 'id', 'nombre');
  }
    ...
    foreach ($imagenes as $imagen) {
      ...
      if ($imagen_item->save()) {
        rename(
          ...
          Yii::getPathOfAlias('webroot') . "/img/produccion/{$this->id}/{$imagen->id}_{$imagen->nombre}"
        );
        $imagen->transaccion = '';
        $imagen->save();
      }
  ...
  {
    return "Producción | {$this->categoria->nombre}";
  }
